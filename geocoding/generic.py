...
class GenericValidator(validators.AddressValidator):
    ...
    def add_data(self):
        if os.path.exists(self.geocode_file):
            ...
            geocode_results.rename(columns = {'lat':f'{self.validator_type}_lat', 'long':f'{self.validator_type}_long', 'confidence': f'{self.validator_type}_confidence'}, inplace=True)
            geocode_results.set_index('TempIDZ', inplace=True)
            self.address_df = geocode_results[[f'{self.validator_type}_lat',f'{self.validator_type}_long',f'{self.validator_type}_confidence']]
        else:
